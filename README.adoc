...
user=> (fetch-in mm [:b :c])
3
user=> (fetch-in mm [:b :z])
...
Or, you may wish to break a string up into a sequence of lines. Then use the `str\->lines` function:
...
Instead, we use the function `only` to emphasize that we are simply _unwrapping_ a single value from
the sequence:
