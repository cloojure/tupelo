...
Clojure has the build-in function `some` to return the first truthy value from a _sequence_
argument. It also has the poorly named function `some?` which returns a literal `true` if a _scalar_
argument satisfies `(not (nil? arg))`. It is easy to confuse these two functions, not only in their
return type but also in the argument they accept (sequence or scalar).  In keeping with the style
for other basic test functions, we provide the function `not-nil?` as the opposite of `nil?`.
