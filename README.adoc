...
Please see the link:http://cloojure.github.io/doc/tupelo/tupelo.parse.html[
tupelo.parse] docs.

==== tupelo.base64 - Convert to/from base64 encoding. 

Please see the link:http://cloojure.github.io/doc/tupelo/tupelo.base64.html[
tupelo.base64] docs.
...
The name `dissoc-entry` is chosen to emphasize that only the map entry
...
conversely, remove all of the undesired elements. Here are a pair of (lazy) functions to perform
these tasks:
...
Of course, these are just thin wrappers around the built-in `clojure.core` functions, but they are
much less ambiguous.  Without these, I am constantly trying to remember which case is correct:

----
  (is (= (filter even? (range 10)) [1 3 5 7 9] )) ; do we     "remove bad" (falsey) items?
                                                  ;   or
  (is (= (filter even? (range 10)) [0 2 4 6 8] )) ; or do we  "keep good" (truthy) items?
----
...
the predicate. It seems to me that `keep-if` and `drop-if` are much more natural names and remove
...
we can copy one from `clojure.contrib.core/seqable`:
