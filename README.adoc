...
call to `(seq some-value)` succeed?  If so, `some-value` may be interpreted as a sequence of values.
Clojure doesn't have a built-in function for this (please note that `seqable?` is different from
`clojure.core/seq?`), but we can copy one from the old `clojure.contrib.core/seqable`:
