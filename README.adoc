...
=== Full API Documentation:  http://cloojure.github.io/doc/tupelo
...
Ever been debugging some code and had trouble printing out intermediate
values?  For example:
...
Note that the same call to `spy` works in either `\->` or `\->>` forms.  The
keyword `:msg` is required just before the message string, as it is this which
allows the `spy` function to detect whether it is in a thread-first or
thread-last form, and thus produce the correct output. As a bonus for debugging,
the value is output using (pr-str ...) so that numbers and strings are
...
all of the forms accept the previous result as the first or last argument.

The built-in threading macro `as\->` can avoid this requirement, but the order of the first
expression and the placeholder symbol is arguably backwards from what users would expect, and there
is often no obvious name to use for the result of the previous form.  Re-using a good idea from
Groovy & Ruby, we simply use the symbol `it` as the placeholder in each expression for the value of
the previous result.
...
The name `dissoc-in` is chosen to emphasize that only the map entry
corresponding to the last key `kZ` is cleared. This also avoids confusion with a
function in the old clojure-contrib library named `dissoc-in` which had the
unpredictable behavior of recursively (& silently) deleting all keys in `keys-vec`
corresponding to empty maps.

=== Extracting *_Only_* Values

The pervasive use of seq's in Clojure means that scalar values often appear
wrapped in a vector or some other sequence type.  One thus sees code like 
`(first some-var)` and it is not always clear that the code is simply "unwrapping" a scalar value,
since there could well be remaining values in the sequence. Indeed, it would be equivalent to use 
`(last some-var)` as well, since first=last for a length-1 sequence. 
...
Within a processing chain, it is often desirable to verify that an intermediate value is within an
expected range or of an expected type. The built-in `assert` function cannot be used for this
purpose since it returns `nil`, and the Prismatic Schema `validate` can only perform a limited
amount of type testing.  The `(validate ...)` function performs arbitrary validation, 
throwing an exception if a non-truthy result is returned:
...
string. In that case, `strcat` is you:
...
Or, you may wish to break a string up into a sequence of lines. Then use the `string\->lines` function:
