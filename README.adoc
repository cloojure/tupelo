...
double) for equality by specifying either the number of significant digits that
must match or the maximum error tolerance allowed:
...
(defn rel= val1 val2 & {:as opts} )
  ...
  equal, or (2) the absolute difference is less than a tolerance value.  Input values
