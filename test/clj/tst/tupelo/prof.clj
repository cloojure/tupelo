(ns tst.tupelo.prof
  ...
  (:require
    ...
    [tupelo.prof :as prof :refer [defnp]]))
...
(dotest
  ...
  (let [fns-to-repeat #{sleep-2 sleep-3 sleep-5 sleep-7
    ...
    ;   #:tst.tupelo.prof{:sleep-2 {:n 13, :mean 0.002509983, :sigma 0.000127452537},
    ...
    ;   :tst.tupelo.prof/sleep-2       13   0.00247 0.00015098
    ;   :tst.tupelo.prof/sleep-3       13   0.00364 0.00020310
    ;   :tst.tupelo.prof/sleep-5       12   0.00570 0.00048202
    ;   :tst.tupelo.prof/sleep-7       11   0.00761 0.00029422
    ;   :tst.tupelo.prof/sleep-11       9   0.01250 0.00088938
    ;   :tst.tupelo.prof/sleep-13       9   0.01464 0.00114652
    ;   :tst.tupelo.prof/sleep-17       8   0.01938 0.00127845
    ;   :tst.tupelo.prof/sleep-77       3   0.07960 0.00099748
    ; -----------------------------------------------------------------------------
    (let [stats-all (prof/stats-get-all)]
      (let [within-tol (fn [val low] ; add 3 millis + 20% as upper bound
                         (<= low val (+ low (* 1.2 low) 0.003)))]
        (is (within-tol (fetch-in stats-all [:tst.tupelo.prof/sleep-2 :mean]) 0.002))
        (is (within-tol (fetch-in stats-all [:tst.tupelo.prof/sleep-3 :mean]) 0.003))
        (is (within-tol (fetch-in stats-all [:tst.tupelo.prof/sleep-5 :mean]) 0.005))
        (is (within-tol (fetch-in stats-all [:tst.tupelo.prof/sleep-7 :mean]) 0.007))
        (is (within-tol (fetch-in stats-all [:tst.tupelo.prof/sleep-11 :mean]) 0.011))
        (is (within-tol (fetch-in stats-all [:tst.tupelo.prof/sleep-13 :mean]) 0.013))
        (is (within-tol (fetch-in stats-all [:tst.tupelo.prof/sleep-17 :mean]) 0.017))
        (is (within-tol (fetch-in stats-all [:tst.tupelo.prof/sleep-77 :mean]) 0.077)))
