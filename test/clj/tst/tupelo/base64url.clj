...
(when (t/is-java-1-8-plus?)
  (dotest
    (let [orig     (byte-array [(byte \A)])
          code-str (b64url/byte-array-encode orig)
          result   (b64url/byte-array-decode code-str)]
  ...
  (dotest
    ...
    (doseq [num-chars [1 2 3 7 20]]
      (let [orig     (str/join (misc/take-dist num-chars char/text))
            code-str (b64url/string-encode orig)
            result   (b64url/string-decode code-str)]
        (is (every? b64url/encoding-char-set (seq code-str)))
        (is= orig result)))
    (is= "KzEyMy1oZWxsbw==" (b64url/string-encode "+123-hello")))

  (dospec 999       ; round-trip-bytes
    (prop/for-all [orig gen/bytes]
      (let [string-b64 (b64url/byte-array-encode orig)
            result     (b64url/byte-array-decode string-b64)]
  ...
  (dospec 999       ; round-trip-string
    (prop/for-all [orig gen/string]
      (let [string-b64 (b64url/string-encode orig)
            result     (b64url/string-decode string-b64)]
        (assert (every? b64url/encoding-char-set (seq string-b64)))
  ...
  (doseq [curr-char char/text]
    ...
    (doseq [prefix ["" "a" "ab" "abc"] ]
      (let [orig-str    (str prefix curr-char)
            enc-str     (b64url/string-encode orig-str)
            dec-str     (b64url/string-decode enc-str) ]
