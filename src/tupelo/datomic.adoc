...
When querying for values in Tupelo Datomic, the fundamental result type is a TupleSet (a Clojure set
containing unique Clojure vectors). Here is an example in action:
...
required.  Most importantly, the `:in` key has been replaced with the `:let` keyword, and the syntax
