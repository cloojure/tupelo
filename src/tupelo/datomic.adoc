...
Entities in Datomic are often specified using their EntityID (EID), which is a globally unique Long
value.  If an entity has an attribute specified as either :db.unique/value or :db.unique/identity,
the entity can also be specified using a LookupRef, which is an attribute-value pair wrapped in a
vector.  In Tupelo Datomic, both EID and LookupRef are examples of an EntitySpec.  Here we verify
that we can find James Bond and retrieve all of his attr-val pairs using both types of EntitySpec:
