...
a structural optimization, and do not control or limit who or how datoms may be accessed.  The
effect of a partition in Datomic is to effectively "pre-sort" all entities in that partition so they
are adjacent in storage, which *may* improve access times for related entities that are often
accessed together.
