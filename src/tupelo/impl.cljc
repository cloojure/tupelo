...
(defmacro let-keys ; #todo -> remove?
  ...
  `let-keys` is safe for typos since `grab` will throw is the requrested map key is not present."
  [ syms the-map & forms]
  `(let
     ~(apply glue
        (for [sym syms
...
(defmacro with-map-fields ; #todo -> README
  "Given a map like {:a 1 :b 2 :c 3} (such as generated by `(label-value-map a b c)`),
  performs safe `let` destructuring using `grab` like:
