...
Display the path from the root to all `:c` nodes:
...
three paths. You can see that the length of the 3 paths matches the position in the tree of the 3 `:c` nodes.
The three `:c` nodes are at the end of the three paths.  Display the last one as Hiccup:
